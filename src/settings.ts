...
export function addFilterSetHeader(
	...
	const filterSetHeader = new Setting(containerEl).setName(header).setHeading().setDesc(description);
	...
	filterSets.forEach((filterSet, i) => {
		createNoteFilterSetInputs(
			...
			(text, notify) => {
				...
				if (!nameUnique && notify) new Notice('Error: Filter Set Name must be unique');

				if (!nameHasCharacters && notify) new Notice('Error: Filter Set Name cannot be blank');
